        ...
        if not inspector.has_table(self.table_name):
            ...
            with self.engine.begin() as conn:
                conn.execute(text(f"""
                    ...
                    CREATE INDEX IF NOT EXISTS idx_finance_code ON {self.table_name} (code);
                    CREATE INDEX IF NOT EXISTS idx_finance_date ON {self.table_name} (report_date);
                """))
        else:
    ...
    def fetch_and_save(self, target_date: str) -> bool:
        ...
        try:
            ...
            with self.engine.begin() as conn:
                ...
                # 再插入新的
                df_save.to_sql(self.table_name, conn, if_exists='append', index=False, method='multi')
