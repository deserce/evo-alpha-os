    ...
    def fetch_etf_kline(self, symbol, start_date=None, end_date=None):
        ...
        try:
            ...
            if start_date is None:
                start_date = datetime.now() - timedelta(days=1095)  # 默认3年

            start_str = start_date.strftime('%Y%m%d')
            end_str = end_date.strftime('%Y%m%d')

            # 使用 fund_etf_hist_em 接口（不需要交易所前缀）
            df = ak.fund_etf_hist_em(symbol=symbol, period="daily", start_date=start_str, end_date=end_str)
